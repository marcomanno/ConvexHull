...
template <class TypeT> class Interval {
...
public:
  ...
  void set(bool _max, const TypeT &_val) { extr_[_max] = _val; }

  const TypeT &operator[](size_t _i) const { return extr_[_i]; }

  TypeT mid() const { return (extr_[0] + extr_[1]) / 2; }
  ...
  Interval<TypeT> &operator*=(const Interval<TypeT> &_oth) {
    if (extr_[0] < _oth.extr_[0])
      extr_[0] = _oth.extr_[0];
    if (extr_[1] > _oth.extr_[1])
      extr_[1] = _oth.extr_[1];
    return *this;
  }
  ...
  TypeT interpolate(TypeT _par) const {
    return _par * extr_[1] + (1 - _par) * extr_[0];
  }
...
template <size_t DimT> struct Range {
  ...
  template <typename CompareT> Range<DimT> &merge(const Range<DimT> &_oth) {
...
public:
  ...
  VectorD<DimT> mid() const { return (extr_[0] + extr_[1]) / 2.; }
  ...
  bool operator!=(const Range<DimT> &_oth) const { return !(_oth == *this); }

  double diag_sq() const { return (extr_[0] - extr_[1]).len_sq(); }
  double diag() const { return std::sqrt(diag_sq()); }

  void set(bool _max, const VectorD<DimT> &_extr) { extr_[_max] = _extr; }

  const VectorD<DimT> &operator[](int _i) const { return extr_[_i]; }
